        ...
        Cats.forEach((subCat) => {
            transactions.forEach((item) => {
                if (parseInt(item.transaction.categoryID) === parseInt(subCat.id)) {
                    recordsLen++;
                    footprints += parseFloat(item.transaction.footprint.carbonEmissionInGrams);
                }
            })
        })
                        ...
                        <ul className="list-group">
                            {
                                categories.map((cat) => {
                                    return (
                                        <li className="list-group-item" key={cat.mainCategoryID} onClick={() => {fillSubCat(cat.mainCategoryID, cat.mainCategory)}}>
                        ...
                        <div className="page-content container note-has-grid">
                            <div className="tab-content bg-transparent">
                                <div className="note-has-grid row">
                                    ...
                                    <h5 className="mb-5 pt-1"><span className="badge text-bg-danger">Average footprint : <b>{average}</b>  kg CO2e </span> </h5> 
                                     {
